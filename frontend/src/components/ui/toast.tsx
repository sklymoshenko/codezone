...
import { Match, splitProps, Switch, type JSX, type ValidComponent } from 'solid-js'
import { Portal } from 'solid-js/web'

const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-2 pr-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--kb-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--kb-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[opened]:animate-in data-[closed]:animate-out data-[swipe=end]:animate-out data-[closed]:fade-out-80 data-[closed]:slide-out-to-right-full data-[opened]:slide-in-from-top-full data-[opened]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
  ...
  return (
    <Portal>
      <ToastPrimitive.Region>
        <ToastPrimitive.List
          class={cn(
            'fixed top-0 z-[100] flex max-h-screen w-full flex-col-reverse gap-2 p-2 sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col md:max-w-[420px]',
            local.class
          )}
          {...others}
        />
...
const ToastTitle = <T extends ValidComponent = 'div'>(
  ...
  return (
    <ToastPrimitive.Title class={cn('text-xs font-semibold', local.class)} {...others} />
  )
}
...
const ToastDescription = <T extends ValidComponent = 'div'>(
  ...
  return (
    <ToastPrimitive.Description
      class={cn('text-xs opacity-90', local.class)}
      {...others}
    />
  )
}
