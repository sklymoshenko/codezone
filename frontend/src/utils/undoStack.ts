...
interface TextInsertCommand extends UndoCommand {
  insertedText: string
  position: number
  timestamp: number
}

interface TextDeleteCommand extends UndoCommand {
  deletedText: string
  position: number
  timestamp: number
}
...
export function createTextInsertCommand(
  ...
  return {
    insertedText: currentInsertedText,
    position,
    ...
    execute() {
      ...
      const newText =
        currentText.slice(0, position) + currentInsertedText + currentText.slice(position)
      setText(newText)
    },
    ...
    undo() {
      ...
      const newText =
        ...
        currentText.slice(position + currentInsertedText.length)
      setText(newText)
    },

    merge(other: UndoCommand): boolean {
      if (!('insertedText' in other) || !('timestamp' in other) || !('position' in other))
        return false

      const otherCmd = other as TextInsertCommand
      ...
      const isSingleChar =
        otherCmd.insertedText.length === 1 && currentInsertedText.length < 50
...
export function createTextDeleteCommand(
  ...
  return {
    deletedText: currentDeletedText,
    ...
    execute() {
      ...
      const newText =
        ...
        currentText.slice(currentPosition + currentDeletedText.length)
      setText(newText)
    },
    ...
    merge(other: UndoCommand): boolean {
      if (!('deletedText' in other) || !('timestamp' in other) || !('position' in other))
        return false

      const otherCmd = other as TextDeleteCommand
      const timeDiff = otherCmd.timestamp - currentTimestamp
      const isBackspace =
        otherCmd.position === currentPosition - otherCmd.deletedText.length
      const isSingleChar =
        otherCmd.deletedText.length === 1 && currentDeletedText.length < 50
